...
from constants import IMAGES_URLS, NUM_PROCS,IMAGES_DIR

def get_download_location():
    try:
        url_input = IMAGES_DIR
    except IndexError:
...
def start_downloading(context,url_list=[],num_procs=NUM_PROCS):
    
    for i in range(NUM_PROCS):
        url = context.get(IMAGES_URLS,True)
        if(str(url).startswith("http")):
            url_list.append()

    start = time.time()

    num_process = int(num_procs)

    images_url = url_list
    run_downloader(num_process, images_url)

    end = time.time()
    print(f"Time taken to download {len(url_list)}")
    print(end - start)
